"""Shared test fixtures for hf-model-search tests."""
...
def populated_conn(conn):
    ...
    db.set_position(conn, "meta-llama/Llama-3.1-8B-Instruct", "ARCHITECTURE", 0, 0, ["decoder-only"])
    ...
    for label in ["decoder-only", "instruction-following", "chat", "7B-class", "Llama-family", "community-favorite"]:
    ...
    db.set_position(conn, "Qwen/Qwen2.5-Coder-1.5B", "ARCHITECTURE", 0, 0, ["decoder-only"])
    ...
    for label in ["decoder-only", "code-generation", "1B-class", "consumer-GPU-viable", "Qwen-family", "code-domain"]:
        aid = db.get_or_create_anchor(conn, label, "CAPABILITY")
    ...
    for label in ["decoder-only", "instruction-following", "GGUF-available", "Llama-family", "quantized"]:
        aid = db.get_or_create_anchor(conn, label, "CAPABILITY")
