...
Seven orthogonal dimensions. Each has a **zero state** placed at the query-frequency mode — the thing most people are looking for. Signed distance from zero encodes direction: negative toward general/simpler/earlier, positive toward specialized/derived/novel.
...
A shared vocabulary of ~110+ semantic labels. Each anchor belongs to one bank, and models link to the anchors that describe them.
...
| QUALITY | `trending`, `high-downloads`, `community-favorite`, `official-release` |
...
**Output:** `PatternResult` with additional anchors, positions, and explicit links.
...
Three-phase pipeline run by `model-atlas-ingest`:

| Phase | What | Cost |
|-------|------|------|
| A: Fetch | Stream HF API sorted by likes, cache raw JSON | Network I/O |
| B: Extract | Tier 1+2 (deterministic + patterns), populate network | CPU only |
| C: Vibes | Tier 3 (Outlines + local LLM) on eligible models | GPU/CPU intensive |

Phases run independently. Phase B can process while Phase A is still fetching. Phase C has a separate quality threshold (default: 50+ likes) to avoid wasting LLM inference on low-quality models.
