...
def _phase_a_huggingface(
    ...
    for model in api.list_models(
        full=True, cardData=True, sort="likes", direction=-1
    ):
        if _shutdown:
            break
...
def phase_c(
    ...
    for row in cursor:
        ...
        try:
            # Build prompt from pre-extracted data
            ...
            if result.summary:
                db.set_metadata(
                    network_conn, model_id, "vibe_summary", result.summary, "str"
                )

            # Store extra anchors with lower confidence
            for anchor_label in result.extra_anchors:
                ...
                if anchor_label:
                    ...
                    db.link_anchor(
                        network_conn, model_id, anchor_id, confidence=0.5
                    )
...
def print_status(ingest_conn: sqlite3.Connection) -> None:
    ...
    for source, info in status["by_source"].items():
        print(f"  [{source}] total={info['total']} A={info['phase_a']} B={info['phase_b']} C={info['phase_c']}")


# ---------------------------------------------------------------------------
# Seed: multi-pass network population
# ---------------------------------------------------------------------------
...
_SEED_PASSES = [
    {
        ...
        "min_likes": 100,
        "min_downloads": 0,
        "min_created": None,  # no date filter
        "pipeline_tags": None,  # all
    },
    {
        "name": "expand",
...
def seed(
    ...
    Args:
        network_conn: Connection to the network database.
        passes: Which passes to run (e.g. ["core", "expand"]). None = all.

    Returns:
        Dict of pass_name -> models_indexed count.
    """
    ...
    # Which model IDs are already in the network?
    existing = {
        row[0]
        for row in network_conn.execute("SELECT model_id FROM models").fetchall()
    }
    logger.info("Seed: %d models already in network", len(existing))

    for pass_def in _SEED_PASSES:
        ...
        min_created = (
            datetime.fromisoformat(min_created_str) if min_created_str else None
        )
        pipeline_tags = pass_def["pipeline_tags"]

        logger.info(
            "Seed pass '%s': %s (min_likes=%d)",
            pass_name, pass_def["description"], min_likes,
        )

        if pipeline_tags:
            # Run one stream per pipeline_tag
            streams = [
                api.list_models(full=True, sort="likes", pipeline_tag=tag)
                for tag in pipeline_tags
            ]
        else:
            streams = [api.list_models(full=True, sort="likes")]
        ...
        for stream in streams:
            ...
            for model in stream:
                ...
                likes = model.likes or 0
                if likes < min_likes:
                    break  # sorted descending, done with this stream
                ...
                if min_created and model.created_at:
                    created = model.created_at
                    if created.tzinfo is None:
                        pass  # compare naive
                    if min_created.tzinfo is None and created.tzinfo is not None:
                        from datetime import timezone
                        min_created = min_created.replace(tzinfo=timezone.utc)
                    if created < min_created:
                        continue

                # Build ModelInput and extract
                try:
                    inp = ModelInput(
                        model_id=model_id,
                        author=model.author or "",
                        ...
                        downloads=downloads,
                        created_at=(
                            str(model.created_at) if model.created_at else None
                        ),
        ...
        logger.info(
            ...
            pass_name, count, stats["total_models"],
        )
...
def run(
    ...
    ingest_conn = db.get_ingest_connection()
    db.init_ingest_db(ingest_conn)
