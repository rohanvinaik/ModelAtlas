"""Standalone Phase D healing worker.
...
Usage:
    python phase_d_worker.py --input d3_shard.jsonl --output d3_results.jsonl
    python phase_d_worker.py --input d3_shard.jsonl --output d3_results.jsonl \
...
def _parse_and_validate(text: str, valid_anchors: set[str] | None = None) -> dict:
    """Parse and validate healing JSON output.

    Returns {"summary": str, "selected_anchors": list[str], "rationale": str}.
    ...
    raw_anchors = data.get("selected_anchors", [])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Standalone Phase D healing worker")
    ...
    with open(args.input) as fin, open(args.output, "w") as fout:
        for line in fin:
            ...
            prompt = item.get("healing_prompt", "")
